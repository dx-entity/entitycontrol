...
class H3CSwitchAction(H3CDevice):
    ...
    def selfInit(self):
        ...
        init multi switch
        """
        snmpVersion = self.bd.h3cSnmp['snmpVersion2']
        snmpComm = self.bd.h3cSnmp['comunityWrite']
        snmpMib_sn = self.bd.h3cSnmp['h3cSwitchMIB']['sn']
        snmpMib_mactable = self.bd.h3cSnmp['h3cSwitchMIB']['mactable']
        snmpMib_table2port = self.bd.h3cSnmp['h3cSwitchMIB']['table2port']
        snmpMib_macAddr = self.bd.h3cSnmp['h3cSwitchMIB']['portmacaddr']
        snmpMib_portName = self.bd.h3cSnmp['h3cSwitchMIB']['portname']
        snmpMib_portStatus = self.bd.h3cSnmp['h3cSwitchMIB']['portstatus']
        ...
        if len(result):
            ...
            sql_insert = 'insert into z_entity_control_test_switch (switch_sn_number, ip) values ("{0}","{1}")'.format(
                sn_num, self.device.ip)
            print sql_insert
        ...
        for i in range(len(port_mac)):
            ...
            if self.db.checkout(sql_check):
                sql_insert = 'insert into z_entity_control_test_port (switch_id,port_name,mac_address,state) values (%s,"%s","%s",%s)' % (
                    self.device.entity_id, name, key, port_status)
                self.db.execsql(sql_insert)

            else:
                sql_update = 'update z_entity_control_test_port set state=%s where mac_address="%s"' % (
                    port_status, key)
        ...
        for i in range(len(mac)):
            ...
            sql_insert = "insert into z_test_entity_control_mactable (mac_learned,port_id,switch_id) values ('%s',%s,%s)" % (
                mac_table, port_id, self.device.entity_id)
            self.db.execsql(sql_insert)
